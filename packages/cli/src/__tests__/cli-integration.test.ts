...
test('strips out builder components by default', async () => {
  ...
  const output = await cli(`compile --from=builder --to=react --styles=style-tag --no-prettier ${filepath}`);
...
test('--builder-components keeps builder components', async () => {
  ...
  const output = await cli(`compile --builder-components --from=builder --to=react --styles=style-tag --no-prettier ${filepath}`);
